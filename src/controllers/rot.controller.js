...
const convert = async (req, res) => {
  const { stringRotation, rotate } = req.body;
  const originalString = stringRotation ? stringRotation.originalString : null;
  try {
    if (originalString && typeof originalString === 'string') {
      const encryption = await RotationService.getRotation(originalString, rotate || DEFAULT_ROT);
      if (encryption && encryption.rotString) {
        return res.json(encryption);
      }
      const rotString = RotationService.encrypt(originalString, rotate || DEFAULT_ROT);
      const response = await RotationService.saveEncryption(originalString, rotString, rotate || DEFAULT_ROT);
